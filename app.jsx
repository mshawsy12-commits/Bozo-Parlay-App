...
const DEFAULT_WEEKS = [
  ...Array.from({ length: 18 }, (_, i) => ({ id: `week-${i + 1}`, label: `Week ${i + 1}` })),
  { id: "wildcard", label: "Wild Card" },
  { id: "divisional", label: "Divisional" },
  { id: "conference", label: "Conference Champ." },
...
const BET_TYPES = [
  ...
  { id: "total", label: "Total (Over/Under)" },
...
const STATUS_META = {
  ...
  loss: { label: "Loss", cls: "st-loss" },
  push: { label: "Push", cls: "st-push" },
};
...
function parlayPayout(picks, stake) {
  ...
  return { legs: values.length, combinedOdds, payout, profit: payout - stake };
}
...
function eventToOption(ev) {
  ...
  return { id: ev.id, isoDate: ev.date, home: home?.team?.displayName || "Home", away: away?.team?.displayName || "Away" };
}
function findEventById(data, id) {
  const ev = (data?.events || []).find((e) => e.id === id);
  if (!ev) return null;
  return { ev, comp: ev.competitions?.[0] };
}
function findEventByTeam(data, teamName, oppName) {
  ...
  for (const ev of events) {
    ...
    if (hasTeam && hasOpp) return { ev, comp };
  }
  for (const ev of events) {
    const comp = ev.competitions?.[0];
    if (!comp) continue;
    const names = comp.competitors.map((c) => normalize(c.team?.displayName + " " + c.team?.shortDisplayName + " " + c.team?.abbreviation));
    if (names.some((n) => wanted && n.includes(wanted))) return { ev, comp };
  }
  return null;
}
...
function gradePick(pick, comp) {
  const { mine, other } = readSides(pick, comp);
  if (!mine || !other) return { status: "pending", finalScore: null, reason: "no-match" };
  ...
  if (pick.betType === "moneyline") {
    if (myScore === oppScore) return { status: "push", finalScore };
    return { status: myScore > oppScore ? "win" : "loss", finalScore };
  }
  if (pick.betType === "spread") {
    const line = Number(pick.line || 0);
    const adjusted = myScore + line;
    if (adjusted === oppScore) return { status: "push", finalScore };
    return { status: adjusted > oppScore ? "win" : "loss", finalScore };
  }
  if (pick.betType === "total") {
    ...
    if (sum === line) return { status: "push", finalScore: finalScoreT };
    const over = sum > line;
    const won = pick.side === "over" ? over : !over;
    return { status: won ? "win" : "loss", finalScore: finalScoreT };
  }
  return { status: "pending", finalScore: null, reason: "unsupported-bet-type" };
}
function liveInfo(pick, comp) {
  const { mine, other } = readSides(pick, comp);
  if (!mine || !other) return null;
  return { clock: comp.status?.displayClock || "", period: comp.status?.period || null, myAbbr: mine.team.abbreviation, oppAbbr: other.team.abbreviation, myScore: mine.score, oppScore: other.score };
}
...
function findOddsApiEvent(events, teamName, oppName) {
  ...
  const comp = {
    status: { type: { completed: !!match.completed, state: match.completed ? "post" : "pre" } },
    competitors: (match.scores || []).map((s) => ({ team: { displayName: s.name, shortDisplayName: s.name, abbreviation: s.name }, score: s.score })),
  };
...
function Avatar({ name }) {
  return <span className="bp-avatar">{initials(name)}</span>;
}
function JesterHat({ size = 40 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" style={{ display: "block" }} aria-hidden="true">
      <path d="M 44 88 C 14 84 -2 62 8 42 C 14 30 26 24 34 32 C 40 38 38 48 30 54 C 34 60 40 64 40 74 C 40 80 42 85 44 88 Z" fill="#ED3455" />
      <circle cx="12" cy="46" r="8" fill="#F2803F" />
      <path d="M 58 86 C 52 54 62 20 90 6 C 102 0 114 4 116 10 C 108 12 96 20 88 34 C 96 40 100 50 94 60 C 88 70 74 78 66 86 Z" fill="#3B93D8" />
      <circle cx="114" cy="9" r="8" fill="#F2803F" />
      <path d="M 76 88 C 106 84 122 62 112 42 C 106 30 94 24 86 32 C 80 38 82 48 90 54 C 86 60 80 64 80 74 C 80 80 78 85 76 88 Z" fill="#5CA633" />
      <circle cx="108" cy="46" r="8" fill="#F2803F" />
      <rect x="18" y="84" width="84" height="22" rx="9" fill="#FFC107" />
    </svg>
  );
}
function BozoFlair({ inline }) {
  return (
    <span className={inline ? "bp-bozo-inline" : "bp-bozo-flair"}>
      <JesterHat size={inline ? 20 : 40} />
    </span>
  );
}
...
function AuthScreen({ claims }) {
    ...
    try {
      ...
      setError(
        e.code === "auth/wrong-password" || e.code === "auth/invalid-credential"
          ...
          ? "That name already has an account — log in instead."
          : e.code === "auth/user-not-found"
          ? "No account for that name yet — sign up instead."
  ...
  return (
    <div className="bp-root" style={{ padding: "36px 24px" }}>
      <div className="bp-auth">
        <div className="bp-title-left" style={{ justifyContent: "center", marginBottom: 8 }}>
          <div className="bp-logo">
            <JesterHat size={22} />
          </div>
          <h1 className="bp-title">
            Bozo <span>Parlay</span>
          </h1>
        </div>
        <div className="bp-tabs" style={{ margin: "18px auto" }}>
          <button className={`bp-tab ${mode === "login" ? "active" : ""}`} onClick={() => setMode("login")}>
            Log In
          </button>
          <button className={`bp-tab ${mode === "signup" ? "active" : ""}`} onClick={() => setMode("signup")}>
            Sign Up
          </button>
        </div>
        ...
        <select value={name} onChange={(e) => setName(e.target.value)}>
          ...
          {(mode === "signup" ? unclaimedNames : ROSTER).map((n) => (
            <option key={n} value={n}>
              {n}
            </option>
          ))}
        </select>
        {mode === "signup" && unclaimedNames.length === 0 && (
          <div className="bp-hint">Everyone's already signed up — switch to Log In.</div>
        )}
        ...
        <button className="bp-btn" onClick={submit} disabled={busy} style={{ marginTop: 10 }}>
          {busy ? "…" : mode === "signup" ? "Create account" : "Log in"}
        </button>
        <div className="bp-hint" style={{ marginTop: 14 }}>
          Each person on the roster gets one account. {ADMIN_NAME} can edit anyone's pick if something needs fixing;
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!roster || !roster.list?.length) {
        roster = { list: ROSTER.map((name) => ({ name })) };
      ...
      if (!wl || !wl.list?.length) {
        wl = { list: DEFAULT_WEEKS };
        await window.BozoDB.setMeta("weeksList", wl);
        for (const w of DEFAULT_WEEKS) {
          await window.BozoDB.setWeekMeta(w.id, { id: w.id, label: w.label, bozo: null, stake: 20 });
        }
      }
  ...
  useEffect(() => {
    ...
    const unsubs = [
      window.BozoDB.watchWeekMeta(currentWeekId, (data) => setWeekMeta(data || { id: currentWeekId, label: currentWeekId, bozo: null })),
      window.BozoDB.watchPicks(currentWeekId, setWeekPicks),
      window.BozoDB.watchVotes(currentWeekId, setWeekVotes),
    ];
  ...
  async function updateOddsApiKey() {
    ...
    if (next === null) return;
    setOddsApiKey(next);
    await window.BozoDB.setMeta("config", { oddsApiKey: next });
  }
  ...
  async function createBetslip() {
    ...
    const entry = { id, label: label.trim(), createdAt: Date.now() };
    const nextList = [...weeksList, entry];
    setWeeksList(nextList);
    await window.BozoDB.setMeta("weeksList", { list: nextList });
    await window.BozoDB.setWeekMeta(id, { id, label: label.trim(), bozo: null, stake: 20 });
  ...
  async function saveWeeklyPick(person, pick) {
    ...
    await window.BozoDB.setPick(currentWeekId, person, { ...pick, updatedAt: Date.now() });
  ...
  async function editStake() {
    ...
    await window.BozoDB.setWeekMeta(currentWeekId, { ...weekMeta, stake: parsed });
  }
  ...
  async function checkScore(person) {
    ...
    setGrading((g) => ({ ...g, [person]: "checking" }));
      ...
      try {
        ...
        if (found && found.comp.status.type.completed) {
          ...
          if (result.status !== "pending") {
            await window.BozoDB.setPick(currentWeekId, person, { ...bet, status: result.status, finalScore: result.finalScore, live: null });
            setGrading((g) => ({ ...g, [person]: "done" }));
            return;
          }
        }
      } catch {
    ...
    try {
      ...
      if (!found || !found.comp) {
        setGrading((g) => ({ ...g, [person]: "not-found" }));
        return;
      }
      ...
      if (state === "post") {
        ...
        await window.BozoDB.setPick(currentWeekId, person, { ...bet, live: info });
        setGrading((g) => ({ ...g, [person]: "live" }));
      } else {
        setGrading((g) => ({ ...g, [person]: "not-final" }));
      }
    } catch {
      setGrading((g) => ({ ...g, [person]: "error" }));
    }
  }
  ...
  const loadSeason = useCallback(async () => {
    ...
    for (const w of weeksList) {
      if (!cache[w.id]) {
        const meta = (await window.BozoDB.getWeekMeta(w.id)) || { label: w.label, bozo: null };
        const picks = await window.BozoDB.getPicksOnce(w.id);
        cache[w.id] = { meta, picks };
      }
    }
  ...
  function computeStandings() {
    const totals = {};
    for (const p of people) totals[p.name] = { win: 0, loss: 0, push: 0, pending: 0, units: 0, bozos: 0 };
    for (const w of weeksList) {
      const entry = seasonCache[w.id];
      if (!entry) continue;
      for (const [person, bet] of Object.entries(entry.picks || {})) {
        if (!totals[person]) totals[person] = { win: 0, loss: 0, push: 0, pending: 0, units: 0, bozos: 0 };
  ...
  if (!dbReady || authUser === undefined) {
    return <div className="bp-loading">Loading…</div>;
  }
  if (!authUser) {
    return <AuthScreen claims={claims} />;
  }
  if (loading || !weekMeta) {
    return <div className="bp-loading">Loading…</div>;
  }
    ...
    <div className="bp-root">
      <div className="bp-scoreboard">
        <div className="bp-title-row">
          ...
          <div className="bp-week-select">
            {tab === "bets" && (
              <select className="bp-select" value={currentWeekId} onChange={(e) => setCurrentWeekId(e.target.value)}>
                {weeksList.map((w) => (
            ...
            <button className="bp-btn" onClick={createBetslip} disabled={creatingBetslip}>
              + New Betslip
            </button>
            <button className="bp-btn ghost" onClick={updateOddsApiKey}>
              {oddsApiKey ? "🔑 Odds API set" : "🔑 Add Odds API key"}
            </button>
            <span className="bp-whoami">
              {loggedInName} {isAdmin && <span className="bp-admin-tag">admin</span>}
              <button className="bp-btn ghost small" onClick={() => window.BozoDB.logOut()}>
                Log out
              </button>
            </span>
          </div>
        ...
        <div className="bp-tabs">
          <button className={`bp-tab ${tab === "bets" ? "active" : ""}`} onClick={() => setTab("bets")}>
            This Week
          </button>
          <button className={`bp-tab ${tab === "standings" ? "active" : ""}`} onClick={() => setTab("standings")}>
            Standings
          </button>
        </div>
      </div>
      ...
      <div className="bp-body">
        <div className="bp-content">
          ...
          {tab === "standings" && (
            <StandingsTab people={people} weeksList={weeksList} seasonCache={seasonCache} totals={totals} loading={seasonLoading} />
          )}
        </div>
      </div>
...
function BozoVoteModal({ weekPicks, weekVotes, weekLabel, loggedInName, castVote, onFinalize, onDismiss }) {
  ...
  return (
    <div className="bp-modal-backdrop" onClick={onDismiss}>
      <div className="bp-modal-panel" onClick={(e) => e.stopPropagation()}>
        <div className="bp-modal-head">
          <span className="bp-modal-title">
            <JesterHat size={26} /> Bozo Vote — {weekLabel}
          </span>
          <button className="bp-btn ghost small" onClick={onDismiss}>
            ✕
          </button>
        </div>
        {losers.length === 0 ? (
          <div className="bp-hint" style={{ marginTop: 8 }}>
            No losing picks this week — nothing to vote on.
          </div>
        ) : (
          <>
            <div className="bp-modal-sub">
              Voting as <b>{loggedInName}</b>. These bets lost this week — vote for who should wear the hat.
            </div>
            <div className="bp-vote-list">
              {losers.map((name) => {
                ...
                return (
                  <button key={name} className={`bp-vote-item ${isMyVote ? "chosen" : ""}`} onClick={() => castVote(name)}>
                    <div className="bp-name-row">
                      <Avatar name={name} />
                      <div>
                        <div className="bp-name">{name}</div>
                        <div className="bp-vote-item-desc">{bet.notes || `${bet.team} vs ${bet.opponent}`}</div>
                      </div>
                    </div>
                    <span className={`bp-vote-count ${isLeading ? "leading" : ""}`}>
            ...
            <div className="bp-modal-footer">
              <span className="bp-hint" style={{ margin: 0 }}>
                {votedCount}/6 voted
              </span>
              <button className="bp-btn" onClick={onFinalize}>
                Crown Bozo
              </button>
            </div>
          </>
...
function ParlayBanner({ weekPicks, stake, canEditStake, editStake }) {
  const result = parlayPayout(weekPicks, stake);
  return (
    <div className="bp-parlay-banner">
      <div className="bp-parlay-main">
        <div className="bp-parlay-label">
          This week's parlay <span className="bp-parlay-sub">({result ? result.legs : 0} leg{result?.legs === 1 ? "" : "s"} · all must hit)</span>
        </div>
        <div className="bp-parlay-figures">
          <div className="bp-parlay-stat">
            <span className="bp-parlay-stat-label">Stake</span>
            <span className="bp-parlay-stat-value">
              ${stake.toFixed(2)}
              {canEditStake && (
                <button className="bp-btn ghost small" onClick={editStake} style={{ marginLeft: 8 }}>
                  Edit
                </button>
              )}
            </span>
          ...
          <div className="bp-parlay-stat">
            <span className="bp-parlay-stat-label">Combined odds</span>
            <span className="bp-parlay-stat-value">{result ? `${result.combinedOdds.toFixed(2)}x` : "—"}</span>
          </div>
          <div className="bp-parlay-stat highlight">
            <span className="bp-parlay-stat-label">Potential payout</span>
            <span className="bp-parlay-stat-value">{result ? `$${result.payout.toFixed(2)}` : "—"}</span>
          </div>
          <div className="bp-parlay-stat">
            <span className="bp-parlay-stat-label">Profit if it hits</span>
            <span className="bp-parlay-stat-value profit">{result ? `+$${result.profit.toFixed(2)}` : "—"}</span>
          </div>
        </div>
      </div>
      {!result && <div className="bp-hint">Add picks below to see this week's potential parlay payout.</div>}
    </div>
  );
}
...
function BetsTab({ people, weekMeta, weekPicks, weekLabel, editingPerson, setEditingPerson, saveWeeklyPick, setManualStatus, clearPick, checkScore, grading, setBozo, canEdit, isAdmin, canEditStake, editStake, oddsApiKey, onOpenVote }) {
  ...
  return (
    <div>
      <ParlayBanner weekPicks={weekPicks} stake={weekMeta.stake ?? 20} canEditStake={canEditStake} editStake={editStake} />
      <div className="bp-week-header">
        <div className="bp-section-title">{weekLabel}</div>
        <div className="bp-week-tools">
          <button className={`bp-btn small ${isVotingWindowOpen() && !weekMeta.bozo ? "" : "ghost"}`} onClick={onOpenVote}>
            🗳️ Bozo Vote{losers.length > 0 ? ` (${losers.length})` : ""}
          </button>
          <span className="bp-bozo-label">
            🤡 Bozo:
            <select className="bp-select" value={weekMeta.bozo || ""} onChange={(e) => setBozo(e.target.value)}>
      ...
      <div className="bp-grid">
        {people.map((p) => {
          ...
          if (isEditing) {
            return (
              <div className="bp-card" key={p.name} onDoubleClick={() => setEditingPerson(null)}>
                <div className="bp-card-head">
                  <div className="bp-name-row">
                    <Avatar name={p.name} />
                    <span className="bp-name">{p.name}</span>
                  </div>
                  <button className="bp-btn ghost small" onClick={() => setEditingPerson(null)}>
                    Cancel
                  </button>
                </div>
            ...
            return (
              ...
              >
                ...
                <span style={{ color: "var(--ink-dim)", fontSize: 13 }}>
                  {editable ? "No pick yet" : "Waiting on their pick"}
                </span>
                {editable && (
                  <button className="bp-btn small" onClick={() => setEditingPerson(p.name)}>
                    Add Pick
                  </button>
                )}
              </div>
...
function BetCard({ title, bet, isBozo, editable, gradingKey, grading, onEdit, onCheckScore, onManualStatus, onClear }) {
  ...
  return (
    <div className={`bp-card ${!editable ? "readonly" : ""}`} onDoubleClick={editable ? onEdit : undefined}>
      {isBozo && <BozoFlair />}
      <div className="bp-card-head">
        <div className="bp-name-row">
          <Avatar name={title} />
        ...
        <Badge status={bet.status} bet={bet} />
      </div>
      <div className="bp-desc">{bet.notes || `${bet.team} vs ${bet.opponent}`}</div>
      <div className="bp-meta">
        <span>{BET_TYPES.find((b) => b.id === bet.betType)?.label}</span>
        <span className="bp-odds">{oddsLabel(bet.odds)}</span>
      </div>
      {bet.live && bet.status === "pending" && (
        <div className="bp-live">
          <span className="bp-live-dot" />
          LIVE Q{bet.live.period} {bet.live.clock} · {bet.live.myAbbr} {bet.live.myScore} — {bet.live.oppAbbr} {bet.live.oppScore}
        </div>
      )}
      {bet.finalScore && <div className="bp-final">Final: {bet.finalScore}</div>}
      <div className="bp-actions" onDoubleClick={(e) => e.stopPropagation()}>
        {canGrade && (
          <button className="bp-btn small" onClick={onCheckScore} disabled={g === "checking"}>
      ...
      {g === "not-found" && (
        <div className="bp-hint">
          Couldn't find/verify this game automatically — grade it yourself:
          <div className="bp-actions" onDoubleClick={(e) => e.stopPropagation()} style={{ marginTop: 6 }}>
            <button className="bp-btn ghost small" onClick={() => onManualStatus("win")}>Win</button>
            <button className="bp-btn ghost small" onClick={() => onManualStatus("loss")}>Loss</button>
            <button className="bp-btn ghost small" onClick={() => onManualStatus("push")}>Push</button>
          </div>
        </div>
      )}
      {g === "error" && <div className="bp-hint">Score check failed — try again or grade manually above.</div>}
      {bet.status !== "pending" && (
        <div className="bp-actions" onDoubleClick={(e) => e.stopPropagation()} style={{ marginTop: 8 }}>
          <button className="bp-btn ghost small" onClick={() => onManualStatus("pending")}>
            Reopen
          </button>
        </div>
      )}
...
function PickForm({ initial, onSubmit, oddsApiKey }) {
  ...
  function submit() {
    ...
    onSubmit({
      betType, team: team.trim(), opponent: opponent.trim(),
      line: line === "" ? null : Number(line), side: betType === "total" ? side : null,
      odds: Number(odds), gameDate, kickoffTime, notes: notes.trim(), espnEventId,
  ...
  return (
    <div className="bp-form">
      <div>
        <label>Search NFL game (optional — auto-fills team, opponent, date & kickoff time)</label>
        <div className="bp-search-row">
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="e.g. Chiefs" onKeyDown={(e) => e.key === "Enter" && runSearch()} />
          <button className="bp-btn small" onClick={runSearch} disabled={searching} style={{ flexShrink: 0 }}>
            {searching ? "…" : "Search"}
          </button>
        </div>
        {searchError && <div className="bp-hint">{searchError}</div>}
        {searchResults && searchResults.length > 0 && (
          <div className="bp-search-results" style={{ marginTop: 6 }}>
            {searchResults.map((opt) => (
              <div className="bp-search-item" key={opt.id}>
                <span>{new Date(opt.isoDate).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span>
                <div className="bp-search-item-teams">
                  <span className="bp-search-pick" onClick={() => pickSide(opt, "away")}>{opt.away}</span>
                  <span style={{ color: "var(--ink-dim)" }}>@</span>
                  <span className="bp-search-pick" onClick={() => pickSide(opt, "home")}>{opt.home}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bp-form-row">
        <div>
          ...
          <input value={team} onChange={(e) => setTeam(e.target.value)} placeholder="Chiefs" />
        </div>
        <div>
          <label>Opponent</label>
          <input value={opponent} onChange={(e) => setOpponent(e.target.value)} placeholder="Bills" />
        </div>
      </div>
      {(betType === "spread" || betType === "total") && (
        <div className="bp-form-row">
          <div>
            <label>{betType === "spread" ? "Spread (e.g. -3.5)" : "Total line"}</label>
      ...
      <div className="bp-form-row-3">
        <div>
          <label>Odds (decimal)</label>
          <input type="number" step="0.01" min="1.01" value={odds} onChange={(e) => setOdds(e.target.value)} placeholder="1.91" />
        </div>
        <div>
      ...
      <div>
        <label>Notes (what's the actual bet — helps for props too)</label>
        <textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Chiefs -3.5, or 'Mahomes over 2.5 passing TDs'" />
      </div>
      {error && <div className="bp-error">{error}</div>}
      <button className="bp-btn" onClick={submit}>Save Bet</button>
    </div>
  );
}
  ...
  return (
    <div className="bp-chart-row">
      {rows.map((r) => {
        ...
        return (
          <div className="bp-chart-col" key={r.name}>
            <span className="bp-chart-value">{r.units >= 0 ? "+" : ""}{r.units.toFixed(1)}</span>
            <div className="bp-chart-bar" style={{ height: `${heightPct}%`, background: color }} />
            <span className="bp-chart-label">{r.name}</span>
          </div>
        );
      })}
    </div>
...
function StandingsTab({ people, weeksList, seasonCache, totals, loading }) {
  const [historySelection, setHistorySelection] = useState("");
  if (loading && Object.keys(seasonCache).length === 0) return <div>Loading season…</div>;

  const rows = people.map((p) => ({ name: p.name, ...totals[p.name] })).sort((a, b) => b.units - a.units || b.win - a.win);
  ...
  const historyBets =
    historySelection && historySelection !== "__all__"
      ? weeksWithData
          .map((w) => {
            const entry = seasonCache[w.id];
            const bet = entry.picks?.[historySelection];
            return bet ? { week: w.label, bet, wasBozo: entry.meta?.bozo === historySelection } : null;
          })
          .filter(Boolean)
      : [];
  ...
  return (
    <div>
      {reigningBozos.length > 0 && (
        <div className="bp-bozo-callout">
          <span className="bp-bozo-callout-icon"><JesterHat size={32} /></span>
          <span className="bp-bozo-callout-text">
            Reigning Bozo{reigningBozos.length > 1 ? "s" : ""}: <b>{reigningBozos.join(", ")}</b> ({maxBozos}x this season)
          </span>
        </div>
      )}

      <div className="bp-standings-card">
        <div className="bp-section-title" style={{ marginBottom: 0 }}>Units by player</div>
        <MiniBarChart rows={rows} />
      </div>

      <div className="bp-section-title">Season Standings</div>
      <table className="bp-lb-table">
        <thead>
          <tr><th></th><th>Player</th><th>W</th><th>L</th><th>P</th><th>Pending</th><th>🤡</th><th>Units</th></tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={r.name}>
              <td className="bp-lb-rank">{i + 1}</td>
              <td className="bp-lb-name"><Avatar name={r.name} />{r.name}</td>
              <td>{r.win}</td><td>{r.loss}</td><td>{r.push}</td><td>{r.pending}</td><td>{r.bozos || 0}</td>
              <td style={{ color: r.units >= 0 ? "var(--cyan)" : "var(--pink)", fontWeight: 600 }}>
      ...
      <div className="bp-section-title" style={{ marginTop: 30 }}>
        Player History
        <select className="bp-select" value={historySelection} onChange={(e) => setHistorySelection(e.target.value)}>
          <option value="">Select a player…</option>
          <option value="__all__">All players (week by week)</option>
          {people.map((p) => (
      ...
      {historySelection === "__all__" && (
        <div style={{ overflowX: "auto" }}>
          <table className="bp-matrix-table">
            <thead>
              <tr>
                <th>Week</th>
                {people.map((p) => <th key={p.name}>{p.name}</th>)}
              </tr>
            </thead>
            <tbody>
              {weeksWithData.length === 0 && (
                <tr><td colSpan={people.length + 1} style={{ textAlign: "center", color: "var(--ink-dim)" }}>No bets logged yet.</td></tr>
              )}
              {weeksWithData.map((w) => {
                const entry = seasonCache[w.id];
                return (
                  <tr key={w.id}>
                    ...
                    {people.map((p) => {
                      ...
                      return (
                        <td key={p.name}>
                          {bet ? (
                            <span className="bp-matrix-cell">
                              <Badge status={bet.status} bet={bet} />
                              {isBozo && <BozoFlair inline />}
                            </span>
                          ) : (
                            <span style={{ color: "var(--ink-dim)" }}>—</span>
                          )}
      ...
      {historySelection && historySelection !== "__all__" && historyBets.length === 0 && (
        <div style={{ color: "var(--ink-dim)", fontSize: 13 }}>No bets logged yet for {historySelection}.</div>
      )}

      {historyBets.map(({ week, bet, wasBozo }) => (
        <div key={week}>
          <div className="bp-history-week">{week} {wasBozo && <BozoFlair inline />}</div>
          <div className="bp-history-row">
            <div>
              <div className="bp-history-desc">{bet.notes || `${bet.team} vs ${bet.opponent}`}</div>
              <div className="bp-history-meta">{oddsLabel(bet.odds)} {bet.finalScore ? `· ${bet.finalScore}` : ""}</div>
            </div>
            <Badge status={bet.status} bet={bet} />
          </div>
        </div>
